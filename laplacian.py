...
def laplacian(adjacencyMatrix, normalize=False):
    ...
    if normalize:
        normalizedDegrees = 1 / torch.sqrt(torch.tensor(
        ...
        return torch.diag(torch.ones(numRows)) - normalizedDegrees
    else:
...
def bestCut(graph):
    ...
    eigenvalues, eigenvectors = torch.linalg.eig(laplacianMatrix)
    eigenvalues = torch.abs(eigenvalues)
    eigenvectors = torch.abs(eigenvectors)
