"""
    Currently I only have support for Cora dataset - feel free to add your own graph dataset.
    ...
    Note on Cora processing:
        ...
        I started from that same dataset, and after pre-processing it the same way as GAT and GCN,
...
def load_graph_data(args, device):
    ...
    load_path = args['train_path']

    if ((dataset_name == DatasetType.CORA.name.lower()) |
            (dataset_name == DatasetType.CiteSeer.name.lower()) |
            (dataset_name == DatasetType.PubMed.name.lower())):  # Cora citation network

        if dataset_name == DatasetType.CORA.name.lower():
            train_range = torch.arange(CORA_TRAIN_RANGE[0], CORA_TRAIN_RANGE[1], dtype=torch.long, device=device)
            val_range = torch.arange(CORA_VAL_RANGE[0], CORA_VAL_RANGE[1], dtype=torch.long, device=device)
            test_range = torch.arange(CORA_TEST_RANGE[0], CORA_TEST_RANGE[1], dtype=torch.long, device=device)
        elif dataset_name == DatasetType.CiteSeer.name.lower():
            train_range = torch.arange(CITESEER_TRAIN_RANGE[0], CITESEER_TRAIN_RANGE[1], dtype=torch.long, device=device)
            val_range = torch.arange(CITESEER_VAL_RANGE[0], CITESEER_VAL_RANGE[1], dtype=torch.long, device=device)
            test_range = torch.arange(CITESEER_TEST_RANGE[0], CITESEER_TEST_RANGE[1], dtype=torch.long, device=device)
        elif dataset_name == DatasetType.PubMed.name.lower():
            train_range = torch.arange(PUBMED_TRAIN_RANGE[0], PUBMED_TRAIN_RANGE[1], dtype=torch.long, device=device)
            val_range = torch.arange(PUBMED_VAL_RANGE[0], PUBMED_VAL_RANGE[1], dtype=torch.long, device=device)
            test_range = torch.arange(PUBMED_TEST_RANGE[0], PUBMED_TEST_RANGE[1], dtype=torch.long, device=device)

        # 2025年7月6日15:58:34 yzx  更改为另一种加载方式
        # shape = (N, FIN), where N is the number of nodes and FIN is the number of input features
        # node_features_csr = pickle_read(os.path.join(load_path, dataset_name, 'node_features.csr'))
        # # shape = (N, 1)
        # node_labels_npy = pickle_read(os.path.join(load_path, dataset_name, 'node_labels.npy'))
        # # shape = (N, number of neighboring nodes) <- this is a dictionary not a matrix!
        # adjacency_list_dict = pickle_read(os.path.join(load_path, dataset_name, 'adjacency_list.dict'))

        data = Planetoid(load_path, dataset_name, split="public").data
        node_features_csr = csr_matrix(data.x)
        # shape = (N, 1)
        node_labels_npy = np.array(data.y)
        # shape = (N, number of neighboring nodes) <- this is a dictionary not a matrix!
        # adjacency_list_dict = data.edge_index
        # 注意原来没有自己给自己的边，为10556  加上就是13264
        adjacency_list_dict = data.edge_index
        ...
        if layer_type == LayerType.IMP3:
            ...
            topology = build_edge_index_addSelf(adjacency_list_dict)
        ...
        # Note: topology is just a fancy way of naming the graph structure dataset
        ...
        # index that help us extract nodes that belong to the train/val and test splits
        # train_index = torch.arange(CORA_TRAIN_RANGE[0], CORA_TRAIN_RANGE[1], dtype=torch.long, device=device)
        # val_index = torch.arange(CORA_VAL_RANGE[0], CORA_VAL_RANGE[1], dtype=torch.long, device=device)
        # test_index = torch.arange(CORA_TEST_RANGE[0], CORA_TEST_RANGE[1], dtype=torch.long, device=device)

        train_index = train_range
        val_index = val_range
        test_index = test_range
        ...
        sys.stderr.write(time.strftime("%m-%d %H:%M:%S") + \
                         "加载数据集结束！详细信息如下: \n数据集名称：{}, 数据集类别数量：{}, 数据集样本个数:{}, 每个样本的特征:{}\n".format(dataset['dataset_name'],\
                        max(dataset['node_labels'])+1, dataset['node_features'].shape[0], dataset['node_features'].shape[1]))
        sys.stderr.flush()
        return dataset
        ...
        #
        # Prepare graph dataset loaders
        #

        # Optimization, do a shortcut in case we only need the test dataset loader
...
def graph_collate_fn(batch):
    ...
    It's important to adjust the node ids in edge index such that they form a consecutive range. Otherwise
...
# All Cora dataset is stored as pickle
...
def build_edge_index(adjacency_list_dict, num_of_nodes, add_self_edges=True):
    ...
    if add_self_edges:
        # 在后面增加 0 到 num_of_nodes  表示 一对一的信息  即 自己
...
def build_edge_index_addSelf(adjacency_list_dict):
    max_len = torch.max(adjacency_list_dict) + 1
    tmp_self = torch.cat((torch.arange(0, max_len).view(1, -1),
                      torch.arange(0, max_len).view(1, -1)), dim=0)
    return torch.cat((adjacency_list_dict, tmp_self), dim=1)
