...
from masterModel import GAT_Model, ClusteringModel
...
from utils.constants import CORA_PATH, DatasetType, LayerType, DATA_DIR_PATH, cora_label_to_color_map, VisualizationType
from utils.visualizations import draw_entropy_histogram
from utils.utils import convert_adj_to_edge_index, name_to_layer_type
from training import get_training_args
...
def profile_gat_implementations(skip_if_profiling_info_cached=False, store_cache=False):
    ...
    training_config['max_loop'] = 500  # IMP1 and IMP2 take more time so better to drop this one lower
    ...
    training_config['test_step'] = None  # and here ^^
    ...
    # Check whether we have memory and timing info already stored in dataset directory
    ...
    if not (cache_exists and skip_if_profiling_info_cached):
        ...
        for run_id in range(num_of_profiling_loops):
            ...
            for gat_layer_imp in gat_layer_implementations:
                ...
                ts = time.time()
                train_model(training_config)  # train and validation
...
def visualize_gat_properties(model_name=r'model_best.model', dataset_name=DatasetType.CORA.name, visualization_type=VisualizationType.ATTENTION):
    """
    Notes on t-SNE:
    ...
    need_config = get_training_args()
    config = {
        'dataset_name': dataset_name,
        'train_path': need_config.train_path,
    ...
    if dataset_name == DatasetType.CORA.name:
        dataLoader = load_graph_data(config, device)
        node_features = dataLoader['node_features']
        topology = dataLoader['topology']
    ...
    model_path = os.path.join(os.path.join('exps/demo', 'model_base'), model_name)
    model_state = torch.load(model_path)

    # 建立一个通用网络框架
    s = ClusteringModel(**vars(get_training_args())).to(device)
    s.load_parameters(model_path, device)
    gat = s.model
    ...
    with torch.no_grad():
        # Step 3: Run predictions and collect the high dimensional dataset
    ...
    if visualization_type == VisualizationType.ATTENTION:
        ...
        for target_node_id in nodes_of_interest_ids:
            ...
            src_nodes_index = torch.eq(target_node_ids, target_node_id)
            source_node_ids = source_nodes[src_nodes_index].cpu().numpy()
            ...
            attention_weights = all_attention_weights[src_nodes_index, head_to_visualize].cpu().numpy()
        ...
        plt.savefig(os.path.join(tmpPath, '{}_embedding_fig.jpg'.format(dataset_name)))
        plt.show()
...
if __name__ == '__main__':
    ...
    if playground_fn == PLAYGROUND.PROFILE_SPARSE:
        ...
        # in dataset/ dir as timing.dict and memory.dict which you can later just load instead of computing again
        ...
        visualize_gat_properties(
            model_name=r'model_best.model',  # set to PPI or CORA model, keep it in sync with the 'dataset_name'
